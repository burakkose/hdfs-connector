...
final case class HdfsWritingSettings(
  ...
  def withBufferSize(bufferSize: Int): HdfsWritingSettings = copy(bufferSize = bufferSize)
  def withReplication(replication: Int): HdfsWritingSettings = copy(replication = replication)
